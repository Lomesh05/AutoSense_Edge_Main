...
enum state_names{ DEVICE_ID, DRIVE_ID, Start_Trip_Call, MQTT, End_Trip_Call, RE_REG_SCREEN, CLEAR,  OTA, TEST, NO_STATE};
...
if(EEPROMr.read(DATA_OFFSET) != NULL)
  ...
  if(client.state() <  0)
    {
      Serial.println("Turn on Yore Mobile Hotsot");
...
void enterKey(char key, int max_len, int x, int y)
{  
  ...
  switch (keypad.getState())
  {
    ...
    case HOLD:
      switch (key)
      {
         ...
         case 'C':
            //Hold 'C' to enter in test mode
                   buff_var[0] = 11;  //11& 12 values are used only fot test purpose and to change the state
                   buff_var[1] = 12;
                  display.clearDisplay();
                  ...
                  display.println("1:Route Addresses");
                  display.display();
                  display.setCursor(0, 24);
                  display.println("2:OTA feature");
                  display.display(); 
                  display.setCursor(0, 36);
                  display.println("3:MQTT Publish");
                  display.display(); 
                   break;  
