void setup_wifi()
{
  ...
  String test = readString(90)+"/api/deviceId/driverId/start";
  Serial.println("");
  Serial.println("");
  Serial.print("test: ");
  Serial.println(test);
...
{
 {
    ...
    html_content += "<label for=\"HTTP1-IP\"><b>Start Route</b></label>";
    html_content += "<input type=\"text\" placeholder=\"Enter start Route\" name=\"HTTP1-IP\" >";

    html_content += "<label for=\"HTTP2-IP\"><b>End Route</b></label>";
    html_content += "<input type=\"text\" placeholder=\"Enter End Route\" name=\"HTTP2-IP\" >";
...
//    
    ...
    server.on("/setting", [](){
      ...
      String start_route = server.arg("HTTP1-IP");
      String end_route = server.arg("HTTP2-IP");
      ...
      if(start_route.length() > 0)
      {
         writeString(start_route,90);
          html_content = "{\"Success\":\"saved to eeprom... reset to boot into new wifi\"}";
         statusCode = 200;
         ESP.restart();
      }

      if(end_route.length() > 0)
      {
         writeString(end_route,120);
          html_content = "{\"Success\":\"saved to eeprom... reset to boot into new wifi\"}";
         statusCode = 200;
         ESP.restart();
      }
